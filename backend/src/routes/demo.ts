...
router.post(
  ...
  async (req: Request, res: Response) => {
    ...
    const demoMatch = DEMO_CONVERSATIONS.find((conv) =>
      conv.user.toLowerCase().includes(message.toLowerCase().slice(0, 20))
    );
    ...
    res.json({
      ...
      data: {
        analysis: {
          ...
          insight,
          recommendation:
            currentPercentile < 30 ? 'BUY' : currentPercentile < 60 ? 'WAIT' : 'AVOID',
        ...
        statistics: {
          ...
          priceTrend: (() => {
            if (prices.length > 8) {
              ...
              return lastPrice !== undefined && oldPrice !== undefined && lastPrice > oldPrice
                ? 'rising'
                : 'falling';
            }
            return 'stable';
    ...
    res.json({
      ...
      data: {
        ...
        monitoringOffer: {
          ...
          confidence:
            "Based on historical data, there's a 65% chance of finding a better price in the next 2 weeks",
        },
      },
    });
  }
